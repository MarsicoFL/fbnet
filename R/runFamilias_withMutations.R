...
#' @param mutModel mutational models supported from Familias, options are: "equal", "Stepwise".
#' @param R mutational rate
#' @param R2 mutational rate for microvariants
#' @param r range (used in stepwise model)
#' @param maleMutModel male model
#' @param maleR male R
#' @param maleR2 male R2
#' @param male.r male range
#' @param femaleMutModel female model
#' @param femaleR female R
#' @param femaleR2 femaleR2
#' @param female.r female range
#' @import Familias
#' @import paramlink
#' @import graphics
#' @export
...
runFamilias_withMutations<-function(ped=NULL,mutModel=c("Equal","Stepwise")[2],R=0.005,R2=5e-7,r=0.5,
                                    maleMutModel=NULL,maleR=NULL,maleR2=NULL,male.r=NULL,
                                    femaleMutModel=NULL,femaleR=NULL,femaleR2=NULL,female.r=NULL){

QP <- geno <- lLociFreq <- NULL
  if(!is.null(ped)){
...
if(is.null(maleMutModel)){
 maleMutModel <- mutModel
 maleR	<- R
 maleR2	<- R2
 male.r	<- r
}

if(is.null(femaleMutModel)){
 femaleMutModel <- mutModel
 femaleR	<- R
 femaleR2	<- R2
 female.r	<- r
}
...
for(i in seq_along(systems)){
 ...
 locus<-FamiliasLocus(frequencies=freqs,
                      allelenames=anames,
                      name=systems[i],
                      maleMutationModel  =maleMutModel,     maleMutationRate=maleR,     maleMutationRange=male.r,    maleMutationRate2=maleR2,
                      femaleMutationModel=femaleMutModel, femaleMutationRate=femaleR, femaleMutationRange=female.r,femaleMutationRate2=femaleR2)
...
if(!is.null(QP)){
 ...
 return(list(res=res,myloci=myloci))
}
