...
#' @description This function has been entirely copied from Familias R package, it has been removed from CRAN. All references and citations about this function should be done to Familias developers. More information could be obtained from: https://www.familias.name/index.html. In fbnet package, this function is only used for dealing with the pedigree object.
#' @param frequencies allele frequencies
#' @param allelenames names
#' @param name system name
#' @param MutationModel model
#' @param MutationRate rates
#' @param MutationRange range
#' @param MutationRate2 rate two, applied for extended stepwise
#' @param MutationMatrix matrix
#' @param Stabilization stabilization factor
#' @param MaxStabilizedMutrate mix factor
#' @param femaleMutationModel for females
#' @param femaleMutationRate rate
#' @param femaleMutationRange range
#' @param femaleMutationRate2 rate 2
#' @param femaleMutationMatrix females matrix
#' @param maleMutationModel male matrix
#' @param maleMutationRate male rate
#' @param maleMutationRange male range
			   ...
			   maleMutationMatrix)
{
    ...
    if(inherits(class(frequencies), "FamiliasLocus")) {
...
pKCompute = function(paj,d){
  ...
  if (ffun(Kmax)>0){
    K = stats::uniroot(ffun,c(0,Kmax))$root
    p = 0.5*(1-sqrt(1-4*K*w))
  } else {
    while (gfun(eps)<0){
      eps = eps/2
    }
    K = stats::uniroot(gfun,c(eps,Kmax))$root
